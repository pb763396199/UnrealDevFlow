...
- [ ] 3.（重测）确认旧的没了、新的在。**关掉这个窗口，重开一个**，敲 `udf --version`，
...
- [ ] 5.（重测）看看帮助现在是不是全中文。敲 `udf --help`，再随便挑两个叶子敲，
  ...
  还剩一处没换：参数给错时 clap 吐的报错（比如 `error: the following required
  arguments were not provided`）还是英文，那串文案在 clap 库内部，换掉要自己
...
- [ ] 8.（重测）让 AI 用一次。让你平时用的那个 AI 助手帮你建一个任务或者查一下状态。
...
- [ ] 10.（重测）切到一个**你现在已经在上面的**任务，比如再敲一次
